...
//Similar to others all input variables&strings are stored as global objects
	...
	if(datafolderexists("Fit_Panel")==0)
		...
		variable/G shellthickness_g, Contact_Radius_display_g
	endif
	setdatafolder root:PanelControl:Fit_panel
	SVAR mess_rc
	...
	NVAR depthcontactfit_g, mechmodel_g, fitlength_g, shellthickness_g, contact_radius_display_g
...
//If using JKR/DMT use the work of adhesion value previously determined as fix (Yes) or leave it as free fitting parameter (No)	
	...
	checkbox contact_radius_display, value=0, title="CRD", variable=root:panelcontrol:fit_panel:contact_radius_display_g, pos={130,395}
...
//Showdwell: Displays dwell parts of force curves(=1); Check_contact: while checking the contact point it offers a closer zoom-in to the current ContPoint (=1)	
	...
	NVAR contact_radius_display_g=root:panelcontrol:fit_panel:contact_radius_display_g
	wave/T listofbasename=root:panelcontrol:singlefcpanel:listofbasenames_g
...
//So far less used, but if separation should be displayed.
	if(showsepa_g==1)
		wave x_axis_tr=$forcecurve+"Sepa_ext"
		wave x_axis_re=$forcecurve+"Sepa_ret"
...
//If showfit is selected in the "display control" mechanical fits are displayed.
	...
	//Fittype=1 means Hertz fit.
		...
		//The name of the hertz fit wave and the recalculated Hertz deformation (HFIT_D) and Hertz-force (HFIT_F) are included.
			string fit_name="HertzFit"+forcecurve
			string HFIT_D=forcecurve+"HFIT_D", HFIT_F=forcecurve+"HFit_F"
		...
		//Fitlimits are later used to set the zoom in the graph.
			wave fitlimits=$forcecurve+"Hertz"
	//Fittype =2/3 == JKR fits.
		...
		//Append a textbox, state the names of the JKR-fit waves and the jkr-adhesion wave.
			textbox/K/N=text0
			string fit_nameJKR="JKRfit"+forcecurve
			string JFIT_F=forcecurve+"JKRFIT_F", JFIT_D=forcecurve+"JKRFIT_D"
			wave jkr_info=$forcecurve+"JKR"
	...
	//fittype=4 is DMT. Since only rarely used only a standard display is yet included.
		elseif(fittype==4)
			string fit_nameDMT="DMTFit"+forcecurve
	...
	//Fittype=5 is Reissner Model
		...
		//Textox is created, Fit-Result waves and Fit-wave names are included.
			...
			string fit_nameREIS="ReisFit"+forcecurve
			string RFIT_D=forcecurve+"RFIT_D", RFIT_F=forcecurve+"RFit_F"
		...
		//Fitlimits are later used to set the zoom in the graph.
			wave fitlimits=$forcecurve+"Reis"			
		endif
...
//In case JKR fits were made.		
	...
	//check for the JKR adhesion information containing wave.
		if(waveexists($forcecurve+"adhfjkr")==1)
	...
	//When no JKR fitting was performed so far take the adhesion force from the "basic" evaluation during baseline correction.
			...
			wave adhesionF2=$forcecurve+"adhf"
			setaxis bottom x_axis_re[adhesionf2[1]]-5e-7,*		
		endif
		setaxis left *,*
		wave cont_tr=$forcecurve+"cont_tr"
		if(waveexists(JKR_INfo)==0)
			cursor/P A $forcecurvecut+"force_ext_co" cont_tr[1]
		endif
	endif
...
//When doing the more elaborated adhesion evalution from the adhesion panel more infos are generated and can be displayed.
	...
	//Include the "range" waves of adhesion. They contain the parts of retrace in mechanically stable contact (-> see adhesion function for details)
		Wave range_start=$forcecurve+"WoA_start"
		wave range_end=$forcecurve+"WoA_end"
		variable i 
...
//This combination is supposed to be used when checking the determined contact points. It offers a close zoom to the contact point& sets a cursor to the found cont.point.
	if(showforce_g==1&&check_contact_g==1&&showdefor_g==1)
		setaxis left,-1e-9,2e-9 //-0.5e-9,2e-9
		setaxis bottom, -2e-8,0.3e-7//-2e-6,2e-6
		wave cont_tr=$forcecurve+"cont_tr"
		cursor/P A $forcecurvecut+"force_ext_co" cont_tr[1]
	endif
...
//Special display... Is supposed to show a log-log plot of force vs deformation. currently buggy.
	...
	//append the log log waves, the log-log fit and a textbox with informations.
		wave exponent=$forcecurve+"expo"
		appendtograph $"log_"+forcecurve+"For" vs $"log_"+Forcecurve+"Defor"
...
//Maugis evaluation needs a unique way to display the force curve.	
	...
	//Set the GUI-Graph as active.
		...
		//The maugis fit and accordingly recalculated force and deformation are included.
			wave fit=$forcecurve+"_MD_fit", Fit_recalc=$forcecurvecut+"MD_DRecalc", fit_Frecalc=$forcecurvecut+"MD_Frecalc"
		elseif(contact_radius_display_g==0)
			wave y_axis_tr=$forcecurvecut+"force_ret_co", x_axis_tr=$forcecurvecut+"defor_ret"
			wave fit_DeforR=$forcecurvecut+"MD_DeforR", Fit_Frecalc=$forcecurvecut+"MD_FRecalc", fit=$forcecurve+"_MD_fit"
		endif
	//If Present...
		if(waveexists(y_axis_tr)==1)
		//Also inculde the corresponding Maugis fit information wave.
			wave MD_info=$forcecurve+"Maugis"
		//Append the contactradius vs force waves to the graph.
			appendtograph y_axis_tr vs x_axis_tr
			iF(contact_radius_display_g==1)
				ModifyGraph mode($forcecurvecut+"contrad_ret")=3
			elseif(contact_radius_display_G==0)
				modifygraph mode($forcecurvecut+"Force_ret_co")=3
			endif
			ModifyGraph zero(bottom)=1
		//Fit[3] is just any result which is not 0 when the fit worked properly.
			if(fit[3]!=0&&contact_radius_display_g==1)
			//Append both the recalced and fitted wave, change there style and add cursors to fit start and end.
				...
				appendtograph fit
				ModifyGraph lsize($forcecurve+"_md_fit")=2
				ModifyGraph rgb($forcecurve+"_md_fit")=(0,65535,0)
				ModifyGraph rgb($forcecurvecut+"MD_drecalc")=(4369,4369,4369)
				cursor/P A $forcecurvecut+"contrad_ret" md_info[9]
				cursor/P B $forcecurvecut+"contrad_ret" md_info[10]
		//If fit didnt work proberly place cursors at 20 and 80% of the contactradius values.
			elseif(fit[3]!=0&&contact_radius_display_g==0)
				appendtograph fit_frecalc vs fit_deforR
				Modifygraph lsize($forcecurvecut+"MD_FRecalc")=2
				ModifyGraph rgb($forcecurvecut+"MD_FRecalc")=(0,65535,0)
			else
				cursor/P A $forcecurvecut+"contrad_ret", round(0.2*numpnts(y_axis_tr))
				cursor/p B $forcecurvecut+"contrad_ret", round(0.8*numpnts(y_axis_tr))
			endif
		...
		//Creat a textbox includeing all fit relevant values and for comparison the formerly determined adhesion force.
			wave adhesionF=$forcecurve+"Adhf", workadhf_md_rips
			NVAR numforcecurve=root:panelcontrol:singlefcpanel:numforcecurve_g
		...
		//Find the point in contactradius where the fit results suggest the Work of adhesion is normalized to. see maugis.
			...
			elseif(contact_radius_display_g==0)
				setaxis bottom -200e-9,*
			endif
			if(V_value!=-1&&contact_radius_display_g==1)
...
//Check if force-waves are present. If not this function doesnt make sense to be done.
	...
	//Fitmodel==1 : JKR; ==2 : DMT -> adjust the endings of the adhesion and Work Of ADHesion waves. *Sing are the WoA-values for the single segments determined.
		if(fitmodel==1)// ???||fitmodel==2)???
			...
			make/N=0/O $forcecurvecut+"WoAdhFSing"
			wave workADHFSing=$forcecurvecut+"WoAdhfSing"
		elseif(fitmodel==2) //was earlier set to 3???
			make/N=5/O $forcecurve+"AdhFDMT"
			wave adhesionF=$forcecurve+"AdhFDMT"
			make/N=2/O $forcecurvecut+"WoAdhFDMT"
			wave workADHF=$forcecurvecut+"WoAdhFDMT"
			make/N=0/O $forcecurvecut+"WoAdhFDMTSing"
			wave workADHFSing=$forcecurvecut+"WoAdhfDMtSing"
		endif	
	...
	//The part of the force and defromation wave are duplicate which are interesting for work of adhesion are duplicate to new waves.
		duplicate/O/R=[zeroretracecontact,zerobaselinejumpout] force_re_co, $Forcecurve+"IntegralF",$Forcecurve+"Integral", $forcecurve+"DerviF", $forcecurve+"Dervi"
		duplicate/O/R=[zeroretracecontact,zerobaselinejumpout] defor_ret, $Forcecurve+"IntegralD",$forcecurve+"DerviD"
	//The duplicate waves are introduced to the function.
		wave integral=$forcecurve+"integral"
		wave integralF=$forcecurve+"integralF"
		wave integralD=$Forcecurve+"IntegralD"
		wave derviF=$forcecurve+"DerviF"
		wave derviD=$forcecurve+"DerviD"
		wave dervi=$forcecurve+"dervi"
	...
	//Make new waves storeing the beginning and end of stable adhesion parts.	
		make/O/n=1 $forcecurve+"WoA_start"
		make/o/N=0 $forcecurve+"WoA_end"
		wave range_start=$forcecurve+"WoA_start"
		wave range_end=$forcecurve+"WoA_end"
		range_start=0
...
///End of contactpointBASEalone code. Know the baseline line equation close to contact is know.
	...
	//In case 4 or 5 is selected the newly calced InvOLS are used to recalc the force-waves.
		...
		//All force-waves are deleted when they exist.
			if(waveexists($forcecurve+"Force_ext")==1)
				killwaves $forcecurve+"Force_ext"
			endif
			if(waveexists($forcecurvecut+"Force_ext_co")==1)	
				killwaves $forcecurvecut+"Force_ext_co"
			endif
			if(waveexists($forcecurve+"Force_ret")==1)
				killwaves $forcecurve+"force_ret"
			endif
		...
		//Could result in an error, when there is no corrected deflection.
			...
			duplicate deflv_ext, $forcecurve+"Force_ext", $forcecurvecut+"Force_ext_co"
			wave force_ext=$forcecurve+"force_ext", force_ext_co=$forcecurvecut+"force_ext_co"
			...
			duplicate deflv_ret, $forcecurve+"Force_ret", $forcecurvecut+"Force_ret_co"
			wave force_ret=$forcecurve+"Force_ret", force_ret_co=$forcecurvecut+"Force_ret_co"
...
//Check for Deformation and Force-waves, since they are requiered.
	...
	//Load the adhesion info wave already calced.
		wave adhesioninfo=$forcecurve+"AdhF"
	//Check if a contact point was found. Actually rather unness. check since without no deformation is existsing...
		...
		//Sometimes the point of adhesion is not the first minimum. For this kind of evaluation one of the first rip off is needed to make a reliable offset for the contact radius.
			...
			if(maxdeformation[numforcecurve_g]*1.01<abs(defor_tr[adhesioninfo[1]]-defor_tr[0]))		//*3 is just random for now. defortr[0] for an potential uncorrected offset in defor on the retrace---_
				...
				v_value+=2
				variable firstminsearch=-1, intervalsize=100		//intervalsize is dependent on sample rate & speed of measurement!
				do
					firstminsearch+=1
	...
	if(NumberByKey("Defor",content,":",";")==1&&numberbykey("Force",content,":",";")==1||numberbykey("ForceCo",content,":",";")==1)
		...
		//	variable errorEmodul=errorHertzFit(forcecurve,Invols,possionratio,sphereradius,sampleradius,springconstant,allowfitoffset)
			if(showfit==1&&wholefolder_g==0)
				...
				TextBox/K/N=text0
				appendtograph jfit_f vs jfit_d
				Modifygraph rgb($forcecurve+"JKRFIT_F")=(0,0,0)
				ModifyGraph rgb($y_name)=(65280,0,0)
				SetAxis bottom -2e-08,defor_tr[upperlimitfit]
				setaxis left 0e-9,y_axis[upperlimitfit]
...
//Reverse of folder_info2list
function folder_list2info(wavetyp,row,savewave)
//Wavetype holds the appendix of the wave. e.g. "_cont_tr" for contact point trace information.
	string wavetyp
//Row specifies the row in the wave where the value is located.
	variable row
...
Function folder_INFOS_as_3Dplot(source1,source2,source3,resolution,source1start,source1end,source2start,source2end,savewave)
	wave source1,source2,source3
	variable resolution,source1start,source1end,source2start,source2end
	string savewave
	make/N=(resolution,resolution)/o $savewave
	wave plot3D=$savewave
	plot3d=0
//	variable Source1Max=wavemax(source1), source1Min=Wavemin(source1), source2Max=wavemax(source2), source2Min=Wavemin(source2)
//	variable deltasource1=(Source1max-source1min)/(resolution-1), deltasource2=(source2max-source2min)/(resolution-1)
	variable Source1Max=source1end, source1Min=source1start, source2Max=source2end, source2Min=source2start
	variable deltasource1=(Source1max-source1min)/(resolution-1), deltasource2=(source2max-source2min)/(resolution-1)
	
	setscale/P x, source1min, deltasource1, plot3d
	setscale/P y, source2min, deltasource2 , plot3d
	variable i, row, column, doublecounter
	for(i=0;i<numpnts(source1);i+=1)
		//for(row=0;row<resolution+1;row+=1)
		if(numtype(source1[i])!=2&&numtype(source2[i])!=2)
			row=trunc(source1[i]/deltasource1)
			column=trunc(source2[i]/deltasource2)
			if(i==128)
				row=row
			endif
			if(row>resolution)
			...
			if(plot3d[row-1][column-1]!=0)
				doublecounter+=1
			endif
			if(abs(plot3d[row-1][column-1])<abs(source3[i]))
				plot3d[row-1][column-1]=source3[i]
			endif	
			row=trunc(source1[i]/deltasource1)
			column=trunc(source2[i]/deltasource2)
		endif
	...
	display
	appendimage plot3d
	ModifyImage $savewave ctab= {wavemin(source3),*,BlueHot256,0},minRGB=NaN,maxRGB=0
	print doublecounter
end
...
//Go through all force curves in the folder.
	...
	//Check the folder content and if Deformation and Force waves are present.
		...
		//Name and load the force, deformation and contact-point result waves.
			string contactstr=Forcecurve+"cont_tr"
			wave y_axis=$y_name, defor_tr=$forcecurve+"Defor_ext"
			wave contact=$contactstr
		//Duplicate the contact part (CP-End) of force and deformation to be able to integrate them.
			duplicate/O/R=[contact[1],numpnts(y_axis)] y_axis, $Forcecurve+"IntegralF",$Forcecurvecut+"Integral"
			duplicate/O/R=[contact[1],numpnts(y_axis)] defor_tr, $Forcecurve+"IntegralD"
			wave integral=$forcecurve+"integral"
			wave integralF=$forcecurve+"integralF"
			wave integralD=$Forcecurve+"IntegralD"
...
function totalenergyofconactalone(forcecurve)
	...
	if(NumberByKey("Defor",content,":",";")==1&&numberbykey("Force",content,":",";")==1||numberbykey("ForceCo",content,":",";")==1)
		...
		wave y_axis=$y_name, defor_tr=$forcecurve+"Defor_ext"
		wave contact=$contactstr
		duplicate/O/R=[contact[1],numpnts(y_axis)] y_axis, $Forcecurve+"IntegralF",$Forcecurve+"Integral"
		duplicate/O/R=[contact[1],numpnts(y_axis)] defor_tr, $Forcecurve+"IntegralD"
		wave integral=$forcecurve+"integral"
		wave integralF=$forcecurve+"integralF"
		wave integralD=$Forcecurve+"IntegralD"
...
//Include the rudimental force curve components deflection volt and displacement (here only zsnsr not raw allowed) (may change in future)
	wave defl_tr=$forcecurve+"DeflV_ext", defl_re=$forcecurve+"DeflV_ret", zsnsr_tr=$forcecurve+"Zsnsr_Ext", zsnsr_re=$forcecurve+"Zsnsr_ret"//, force_tr=$forcecurvecut+"Force_ext", force_re=$forcecurvecut+"Force_ret"
...
// ==0 -> cursor A is on the graph
	if(cursortouse==0)
		...
		//Check for potential force and deformation waves and cut them too.
			...
			if(waveexists($forcecurve+"Force_ext")==1)
				deletepoints acsr, numpnts($forcecurve+"Force_ext"), $forcecurve+"Force_ext"
			endif
			if(waveexists($forcecurve+"Defor_ext")==1)
				deletepoints acsr, numpnts($forcecurve+"defor_ext"), $forcecurve+"Defor_ext"
			endif
	//The same for the retrace if "only retrace" is selected.
		...
		//Note that retrace is point-wise order from max piezo extension -> max contraction.
			...
			if(waveexists($forcecurve+"Force_ret")==1)
				deletepoints acsr, numpnts($forcecurve+"Force_ret"), $forcecurve+"Force_ret"
			endif
			if(waveexists($forcecurve+"Defor_ret")==1)
				deletepoints acsr, numpnts($forcecurve+"defor_ret"), $forcecurve+"Defor_ret"
			endif
		endif
	...
	elseif(cursortouse==1)
		if(retraceonly_g!=1)
			...
			if(waveexists($forcecurvecut+"Force_ret")==1)
				deletepoints bcsr, numpnts($forcecurve+"Force_ret"), $forcecurve+"Force_ret"
			endif
			if(waveexists($forcecurvecut+"Defor_ret")==1)
				deletepoints bcsr, numpnts($forcecurve+"defor_ret"), $forcecurve+"Defor_ret"
			endif
		endif
// ==2 -> no or both cursors are on the graph -> take the distance value given to the function.
	...
	//Right now only for retrace: find the point corresponding to the distance and delete the points.
		if(retraceonly_g==1)
			...
			if(waveexists($forcecurve+"Force_ret")==1)
				deletepoints v_value, numpnts($forcecurve+"force_ret"), $forcecurve+"force_ret"
			endif
			if(waveexists($forcecurve+"Defor_ret")==1)
				deletepoints v_value, numpnts($forcecurve+"defor_ret"), $forcecurve+"Defor_ret"
			endif
		endif	
	endif
end
...
//in case non existing create all the result waves.
	if(waveexists($adhesion_folder)==0)
		...
		if(mechmodel==1)
			make/N=(numpnts(listofbasename)) $"workofadhesion_JKR"//, $"WorkAdh_byJKRModel"
		else
			make/N=(numpnts(listofbasename)) $"workofadhesion_DMT"//, $"WorkAdh_byDMTModel"
		endif
	endif
